...
export function FamilySubscriptionForm({ onClose }: FamilySubscriptionFormProps) {
  const [currentTab, setCurrentTab] = useState('assure');
  ...
  useEffect(() => {
    const generateCodes = async () => {
      try {
        ...
        const newContractNumber = `FAM${year}-${random1}-${random2}-${random3}/MAC`;
  ...
  const addFamilyMember = () => {
    setFamilyMembers([...familyMembers, { first_name: '', last_name: '', birth_date: '', birth_place: '', relationship: 'enfant', gender: 'M' }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const raisonSociale = `${lastName} ${firstName}`;
      ...
      // 3. Create spouse as beneficiary if married
      ...
      // Log audit
      auditLog.create('contract', `Nouvelle souscription famille ${contractNumber} - ${raisonSociale}`, contractData.id);
  ...
  const handleGeneratePDF = () => {
    const pdfData: FicheSubscriptionPDFData = {
      client_code: clientCode,
      contract_number: contractNumber,
      raison_sociale: `${lastName} ${firstName}`,
      address: address,
      insured: {
        last_name: lastName,
        first_name: firstName,
        ...
        <TabsContent value="famille" className="space-y-4 mt-6">
          <div className="form-section">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Ajoutez les enfants et autres personnes à charge à couvrir par l'assurance.
            </p>
            ...
            {familyMembers.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground border border-dashed border-border rounded-lg">
                ...
                <p className="text-sm">Cliquez sur le bouton ci-dessous pour ajouter un membre</p>
              </div>
            ) : (
          ...
          <div className="form-section">
            ...
            <div className="flex items-start gap-3 p-4 bg-muted/30 rounded-lg">
              ...
              <Label htmlFor="engagement" className="text-sm text-muted-foreground cursor-pointer leading-relaxed">
                ...
                conformément aux articles 113-8 et 113-9 du Code des Assurances.
              </Label>
            </div>
          </div>
        </TabsContent>
